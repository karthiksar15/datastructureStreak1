class Sep0226 {
    ...
    public int stoneGameII(int[] piles) {
        ...
        // dp[i][m] = max stones a player can get starting at index i with M = m
        ...
        for (int i = n - 1; i >= 0; i--) {
            for (int m = 1; m <= n; m++) {
                ...
                // OPTIMIZATION: If you can take all remaining stones, just take them all!
                if (i + 2 * m >= n) {
                    dp[i][m] = suffixSum[i];
                } else {
                    // Otherwise, try all possible valid moves x
                    for (int x = 1; x <= 2 * m; x++) {
